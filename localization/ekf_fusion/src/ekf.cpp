...
void EKF::predict(const Eigen::Vector3d& acc, const Eigen::Vector3d& ang, double dt) {
    ...
    */
    ...
    F.block<3,3>(0,6) = -0.5 * state_.R * Sophus::SO3d::hat(acc) *dt*dt;

    // dv/dR = dRa/dR * dt
    F.block<3,3>(3,6) = -state_.R * Sophus::SO3d::hat(acc) *dt;

    // dR/dR = exp(dw)
    F.block<3,3>(6,6) = Sophus::SO3d::exp(ang * dt);
...
void updatePose(const Eigen::Vector3d& p_meas, const Sophus::SO3d& R_meas, const Eigen::Matrix<double 6, 6>& R_cov) {
    ...
    Eigen::Vector3d ori_err = (state_.R.invers() * R_meas).log(); 
    ...
    Eigen::Matrix<double,9,6> K = P_*H.transpose()*S.invers();
    // dx = x - x_meas
